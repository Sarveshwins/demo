...
(0, _utils.loadAppConfig)().then(function (appConfig) {
  var currentAppName = appConfig.name || appConfig.expo && appConfig.expo.name;
  ...
  _commander2.default.version(projectVersion).arguments('[newName]').option('-b, --bundleID [value]', 'Set custom bundle identifier eg. "com.junedomingo.travelapp"').action(function (argName) {
    ...
    var resolveFoldersAndFiles = function resolveFoldersAndFiles() {
      var promises = listOfFoldersAndFiles.map(function (element) {
        var dest = element.replace(new RegExp(nS_CurrentAppName, 'i'), nS_NewName);

        var successMsg = '/' + dest + ' ' + _colors2.default.green('RENAMED');

        var src = _path2.default.join(_utils.__dirname, element);
        var dst = _path2.default.join(_utils.__dirname, dest);

        var move = _shelljs2.default.mv('-f', src, dst);

        if (move.code === 0) {
          console.log(successMsg);
        } else {
          console.log(_colors2.default.yellow("Ignore above error if this file doesn't exist"));
        }
      });
        ...
        (0, _utils.loadAndroidManifest)().then(function ($data) {
          ...
          var promises = androidEnvs.map(function (env) {
            return new Promise(function (envResolve) {
              ...
              var successMsg = newBundlePath + ' ' + _colors2.default.green('BUNDLE IDENTIFIER CHANGED');
    ...
    var gitUpdateChanges = function gitUpdateChanges() {
      _shelljs2.default.exec('git add . ');
    };

    var run = function run() {
      return Promise.resolve().then(validatePaths).then(resolveFoldersAndFiles).then(resolveFilesToModifyContent).then(resolveJavaFiles).then(gitUpdateChanges).then(cleanBuilds).then(function () {
